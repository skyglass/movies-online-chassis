...
gradle.projectsEvaluated {
    val assemble = tasks.findByPath(":main:assemble")
    for (task in tasks) {
...
val todoModules =
        mapOf(
                "series-01-live-project-01-configuring-observability" to listOf("service-chassis-health-check", "service-chassis-distributed-tracing", "metrics")
                ,"series-01-live-project-02-implementing-security" to listOf("web-security", "service-template-main")
        )
...
tasks.findByPath(":main:bootRun")!!.dependsOn(":sqlComposeUp")
tasks.findByPath(":main:bootRun")!!.dependsOn(":sqlAndKeycloakComposeUp")
